...
def aumento_salarial(salario, porcentagem):
    ...
    acresimo = salario * porcentagem/100
    novo_salario = acresimo + salario
...
def dias_para_segundos(dias, horas, minutos, segundos):
    ...
    tempo = dias * 24 * 3600 + horas * 60 * 60 + minutos * 60 + segundos
    return tempo
...
def salario(valor_hora, horas_mensais):
    """ Recebe quanto ganha por hora e quantas horas trabalha ao mês,
    ...
    todo_meu_pau = valor_hora * horas_mensais
    to_dando = (8/100) * todo_meu_pau
    te_peguei = (11/100) * todo_meu_pau
    sua_cadela = (5/100) * todo_meu_pau

    return todo_meu_pau - to_dando - te_peguei - sua_cadela


def tinta(metros_pintar):
    """ Recebe quantos metros quadrados precisa pintar,
    e retorna a quantidade de latas de tinta a comprar.True
...
def decompor_numero(numero):
    ...
    abr = numero // 100
    aco = (numero - (abr * 100))//10
    peg = numero - (abr * 100) - (aco * 10)
